...
public:
	...
	std::vector<migration_cell_t> do_genetic (individual_t ind) {
		...
		individual_t best = p.get_best_ind();
	...
	individual_t tournament () {
		individual_t ind = p.get_best_ind();
	...
	inline std::vector<migration_cell_t> get_pages_to_migrate (std::map<pid_t, page_table_t> & page_ts) {
		// Generates an individual for current state and calculates its performance
		individual_t ind(page_ts);
		return do_genetic(ind);
	}

	inline std::vector<migration_cell_t> get_threads_to_migrate (std::map<pid_t, page_table_t> & page_ts) {
